...
//------------------------------------------------------------------------------------------	
	if(isset($_REQUEST['user']))
	{
		...
		$clave	 = $_REQUEST['pass'];
		$consulta = "SELECT * FROM `usuarios` WHERE usuarios.user = '$usuario' and usuarios.pass = '$clave' and usuarios.admin = 1";
		...
		if(mysqli_num_rows($resultado) <> 0){
		    ...
		    $resultado = mysqli_query($link, $consulta);
		    if(mysqli_num_rows($resultado) <> 0){
		        echo "<script>alert('ERROR DE LOGIN: Ingrese su usuario y contraseña e intente nuevamente.')</script>";
		        enviar("index.php");
		    }
		    else{
